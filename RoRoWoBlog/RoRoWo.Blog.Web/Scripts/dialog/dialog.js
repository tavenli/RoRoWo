...
Dialog.prototype = {
    ...
    'init': function() {
        ...
        /* 创建标题文本 */
        this.dom.title = $('<span class="dialog_title"></span>').get(0);
            ...
            case 'message':
                ...
                switch (type) {
                    ...
                    case 'warning':
                        ...
                        _ok_button = $('<input type="button" class="cbtn1" value="' + button_name + '" />');
                    ...
                    case 'confirm':
                        ...
                        _yes_button = $('<input type="button" class="cbtn1" value="' + yes_button_name + '" />');
                        _no_button = $('<input type="button" class="cbtn2" value="' + no_button_name + '" />');
...
function ajax_notice(id, title, url, width) {
    if (!width) {
        width = 400;
    }
    var d = DialogManager.create(id);
...
//显示一个正在等待的消息
function loading_form(id, title, _text, width) {
    if (!width) {
        width = 400;
    }
    var d = DialogManager.create(id);
...
//显示一个提示消息
function message_notice(id, title, _text, width) {
    if (!width) {
        width = 400;
    }
    var d = DialogManager.create(id);
...
//显示一个带确定、取消按钮的消息
function message_confirm(id, title, _text, width) {
    if (!width) {
        width = 400;
    }
    var d = DialogManager.create(id);
...
//显示一个内容为自定义HTML内容的消息
function html_form(id, title, _html, width) {
    if (!width) {
        width = 400;
    }
    var d = DialogManager.create(id);
